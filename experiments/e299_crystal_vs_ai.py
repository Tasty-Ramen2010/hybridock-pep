...
Run: OMP_NUM_THREADS=1 python experiments/e299_crystal_vs_ai.py
"""
...
spec = importlib.util.spec_from_file_location("e296", os.path.join(ROOT, "experiments/e296_ifp_rigorous.py"))
...
import types
src = open(os.path.join(ROOT, "experiments/e296_ifp_rigorous.py")).read()
src = src.split("def build()")[0]  # keep only constants + receptor_atoms_and_seq + peptide_atoms + rich_ifp
mod = types.ModuleType("e296p"); mod.__dict__["__file__"]=os.path.join(ROOT,"experiments/e296_ifp_rigorous.py"); exec(compile(src, "e296p", "exec"), mod.__dict__)
