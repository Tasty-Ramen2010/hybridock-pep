...
# Band-parameterized real-pose campaign. Usage: e176_realpose_queue.py <band> <n_samples>
#   band ∈ {short(≤8), med(9-12), long(13-16), vlong(≥17)}; excludes complexes already in e93/e154 + this band's OUT.
_BANDS = {"short": (1, 8), "med": (9, 12), "long": (13, 16), "vlong": (17, 999)}
BAND = sys.argv[1] if len(sys.argv) > 1 else "vlong"
NSAMPLES = sys.argv[2] if len(sys.argv) > 2 else "250"
_LO, _HI = _BANDS[BAND]
WORK = ROOT / "runs" / f"e176_{BAND}"
OUT = ROOT / "data" / f"e176_{BAND}_n{NSAMPLES}.jsonl"
...
def candidates():
    ...
    out = []
    for r in rows:
        if not (_LO <= r["length"] <= _HI):     # this band only
            continue
...
def main():
    ...
    print(f"=== E176 {BAND} N={NSAMPLES} + ranker-best: {len(done)} done, {len(todo)} to do ===", flush=True)
    t0 = time.time(); n = 0
    for c in todo:
        try:
            rec = run_one(c)
    ...
    print(f"=== E176 {BAND} done: {n} new ===", flush=True)
