"""Miyazawa-Jernigan residue-residue contact potential (1996).

A 20x20 statistical contact-energy matrix derived from residue-residue contact
frequencies in known protein structures. More NEGATIVE = more favourable contact
(e.g. Trp-Trp hydrophobic burial), less negative / positive = unfavourable
(e.g. Lys-Lys like-charge). Used by geometry_features.compute_geometry_features to
weight peptide-receptor contacts by their chemical identity — the per-contact ENERGY
signal that a bare contact COUNT misses (captures hotspot residues).

MJ_ENERGY[(a, b)] is the contact energy (kT units) for single-letter residues a, b.
"""
...
MJ_ENERGY: dict[tuple[str, str], float] = {}
_rows = [list(map(float, r.split())) for r in _MJ_RAW.strip().splitlines()]
for _a, _ia in enumerate(_MJ_ORDER):
    for _b, _ib in enumerate(_MJ_ORDER):
        MJ_ENERGY[(_ia, _ib)] = _rows[_a][_b]
