...
OUT = ROOT / "data" / "e175_n100_vlong.jsonl"
...
def candidates():
    ...
    # exclude complexes we ALREADY have real poses for (e93 + e154) — add only NEW vlong
    ...
    cands = []
    for r in rows:
        if r["length"] < 17:                       # VLONG ONLY
            continue
        if r["pdb"].lower() in have:               # skip existing
            continue
