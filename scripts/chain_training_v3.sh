...
#
# Prerequisites:
#   - rapidock conda env with train_lastlayer.py at third_party/RAPiDock_finetuned/
#   - Pretrained checkpoint at path set in PRETRAINED_CKPT below
#   - PepPC curated CSVs at datasets/training_formatted_peppc/
#
set -euo pipefail
...
echo "[v3-chain] Train CSV:  $TRAIN_CSV"
echo "[v3-chain] Val CSV:    $VAL_CSV"
echo ""
...
if [[ ! -f "$TRAIN_CSV" ]]; then
    echo "ERROR: train CSV not found: $TRAIN_CSV"; exit 1
fi
if [[ ! -f "$VAL_CSV" ]]; then
    echo "ERROR: val CSV not found: $VAL_CSV"; exit 1
fi

# ─── Helper: report checkpoint size + confirm it exists ───────────────────────
ckpt_info() {
    ...
    if [[ -f "$path" ]]; then
        ...
        echo "ERROR: expected checkpoint not found: $path"; exit 1
    fi
}

# ─────────────────────────────────────────────────────────────────────────────
# Phase 1: score heads + cross_convs.2/3 ONLY
#   Goal: adapt receptor-recognition cross-attention, intra_convs entirely frozen
#   LR=2e-5, warmup=5, epochs=20, EMA=0.9995, WD=1e-6
#   No pretrained-reg in P1 (all non-heads are frozen — can't drift anyway)
# ─────────────────────────────────────────────────────────────────────────────
echo "=================================================================="
...
#   cross_convs.3=1.0×, cross_convs.2=0.70×, intra_convs.3=0.15×
#   Weak L2 pretrained-weight regularization on intra_convs + early layers
#   LR=5e-6, warmup=6, epochs=40, EMA=0.9997, WD=1e-5
# ─────────────────────────────────────────────────────────────────────────────
echo ""
echo "=================================================================="
...
# ─────────────────────────────────────────────────────────────────────────────
# Phase 3: full model except ESM; cosine LR; layerwise decay; checkpoint-per-epoch after ep20
#   peak LR=7e-6 → 1e-7 cosine, warmup=10, epochs=80
#   EMA=0.9999, layerwise decay (heads 1.0×, late 0.5×, mid 0.2×, early 0.05×)
#   Continued weak pretrained-weight regularization
# ─────────────────────────────────────────────────────────────────────────────
echo ""
echo "=================================================================="
...
echo ""
echo "  Late-epoch checkpoints (every epoch from ep20) are in:"
...
echo "    3. Optional: average late-stable P3 checkpoints for robustness"
echo "=================================================================="
