...
def main():
    ...
    # vlong sub: 925 + PPIKB-vlong (geom-complete)
    aug = []
    ...
    Xv = np.vstack([X, np.array([a[0] for a in aug])]); yv = np.concatenate([y, np.array([a[1] for a in aug])])
    Lv = np.concatenate([L, np.array([a[2] for a in aug])])
    rgv = regs_of(Xv, Lv); vlong_model = e202._hgb().fit(apply_regs(Xv, Lv, rgv), yv)
    ...
    bundle = {"model": main_model, "size_regs": serialize(rg), "feature_order": FEATURE_NAMES,
              ...
              "vlong_n_train": len(tr) + len(aug)}
    out = ROOT / "data/affinity_crystal_sizefix.joblib"
    joblib.dump(bundle, out)
    print(f"saved routed crystal: main n={len(tr)}, vlong-sub n={len(tr)+len(aug)} (+{len(aug)} PPIKB-vlong), threshold L>=17")
