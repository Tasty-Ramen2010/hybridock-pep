...
def get_score(data, tr_update, tr_sigma, rot_update, rot_sigma, tor_backbone_updates, tor_backbone_sigma, tor_sidechain_updates, tor_sidechain_sigma):
    ...
    # Store per-sample sigmas so the loss can be SIGMA-NORMALIZED (DiffDock/DiffPepDock
    # convention). Without this, plain MSE on the score (~1/sigma) is dominated by
    # low-sigma samples, starving coarse PLACEMENT. Only set during training (get_score
    # is not called at inference), so this is safe.
    data.tr_sigma_val = float(tr_sigma)
    data.rot_sigma_val = float(rot_sigma)
    data.tor_bb_sigma_val = float(tor_backbone_sigma)
    data.tor_sc_sigma_val = float(tor_sidechain_sigma)
